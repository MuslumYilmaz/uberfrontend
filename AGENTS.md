...
- Test quality: new or updated tests must protect a real behavior boundary such as user-visible behavior, business invariants, regression-prone state transitions, API/contracts, or critical rendering/routing branches. Avoid low-signal tests that only prove construction, static copy, or DOM shape unless that exact shell/rendering contract is the thing being protected. Test names and failures should make the purpose obvious.
- Verification contract: for every non-trivial change, run the closest meaningful checks before closeout and report them honestly. Final responses must state what was verified, what was not verified, and any residual risk. If a change touches a critical path and no meaningful verification exists yet, add or strengthen the nearest high-signal test instead of relying on manual hope.
- Critical paths: treat auth/session, billing/premium access, progress/xp/streak/dashboard, question or registry generation integrity, and route/SEO/prerender contracts as higher-risk surfaces that need stronger verification than ordinary copy or layout edits.
