...
describe('InterviewQuestionsLandingComponent', () => {
  ...
  it('renders HTML and CSS answer-first coverage while preserving format-category links', async () => {
    ...
    expect(fixture.nativeElement.querySelector('a[href="/css/trivia/css-position-sticky-not-working"]')).toBeTruthy();
    ...
    const htmlCssNavbarLinks = Array.from(
      fixture.nativeElement.querySelectorAll('a[href="/css/coding/css-flexbox-navbar"]') as NodeListOf<HTMLAnchorElement>
    );
  ...
  it('renders CSS-only answer-first coverage and schema mentions', async () => {
    ...
    const cssNavbarLinks = Array.from(
      fixture.nativeElement.querySelectorAll('a[href="/css/coding/css-flexbox-navbar"]') as NodeListOf<HTMLAnchorElement>
    );
    expect(cssNavbarLinks.some((link) => /responsive navbar|Flexbox navbar/i.test(link.textContent || ''))).toBeTrue();
